...
vec2 morphVertex(vec2 vertex, float morphLerpK)
{
...
vec4 morphVertexInWorld(vec4 vertex)
{
  vec4 groundVertex = vec4(vertex.x, 0.0, vertex.z, vertex.w);
  vec3 viewVector = vec3(viewMatrix * groundVertex);
  float distance = length(viewVector);
  float morphK = findMorphK(distance);
  vec2 morphed = morphVertex(vertex.xz, morphK);

  return vec4(morphed.x, vertex.y, morphed.y, vertex.w);
}
...
void main()
{
  ...
  if(mod(coordinate.x, 2.0) == 1.0 || mod(coordinate.y, 2.0) == 1.0) {
    modelPosition = morphVertexInWorld(modelPosition);
  }
